  ...
  async (req, res) => {
    ...
    if (fxQuoteId) {
      ...
      if (!response.ok) {
        ...
        throw new ApiError(
          500,
          errorData?.error?.message || "Stripe error",
          "StripeError"
        );
      }
...
legacyNextApiRouter.post(
  ...
  async (req, res) => {
    const { items, metadata, customerEmail } = req.body as z.infer<
...
function getLockDuration(fromCurrencies: string[], requestedDuration = "hour") {
  const hasRestricted = fromCurrencies.some((c) =>
...
const fxQuotePostSchema = z.object({
  ...
  fromCurrencies: z
    ...
    .nonempty()
    .optional()
    .default(["usd", "eur", "cad", "aud", "jpy"]),
  lockDuration: z.string().optional().default("hour"),
});

legacyNextApiRouter.post(
  "/fx-quote",
  validateBody(fxQuotePostSchema),
  async (req, res) => {
    if (!env.STRIPE_SECRET_KEY)
      throw new ApiError(500, "STRIPE_SECRET_KEY is not set", "ConfigError");
    const { toCurrency, fromCurrencies, lockDuration } = req.body as z.infer<
      typeof fxQuotePostSchema
    >;
    ...
    if (!response.ok) {
      ...
      throw new ApiError(
        400,
        errorData?.error?.message || "FX quote error",
        "StripeError"
      );
    }
...
legacyNextApiRouter.get("/fx-quote", async (req, res) => {
  ...
  if (!quoteId)
    throw new ApiError(400, "FX quote ID is required", "BadRequest");
  if (!env.STRIPE_SECRET_KEY)
    throw new ApiError(500, "STRIPE_SECRET_KEY is not set", "ConfigError");

  const response = await fetch(
    `https://api.stripe.com/v1/fx_quotes/${quoteId}`,
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${env.STRIPE_SECRET_KEY}`,
...
legacyNextApiRouter.post(
  ...
  async (req, res) => {
    const { email, locale, fields } = req.body as z.infer<
    ...
    if (!env.SYSTEME_API_KEY) {
      throw new ApiError(
        500,
        "Newsletter service is not configured (SYSTEME_API_KEY missing)",
        "ConfigError"
      );
    }
    ...
    if (!response.ok) {
      ...
      throw new ApiError(
        response.status,
        errorText || "Newsletter subscription failed",
        "NewsletterError"
      );
    }
...
legacyNextApiRouter.post(
  ...
  async (req, res) => {
    ...
    const shippingAddress = (paymentIntent as any).shipping
      ? {
          name:
            (paymentIntent as any).shipping.name || customerName || "Customer",
    ...
    res.json({
      ...
      paymentEmailMessageId: paymentEmailResult.success
